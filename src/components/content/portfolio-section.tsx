...
export function PortfolioSection() {
  const projects = [
    ...
    },
    {
      title: "SystemPackage",
      description:
        "SystemPackage es una plataforma de rastreo y gestión de paquetes que permite a los usuarios monitorear envíos en tiempo real, gestionar órdenes y recibir notificaciones automáticas sobre el estado de sus entregas.",
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 text-gray-100 shadow-xl p-6 sm:p-8 rounded-xl">
      <div className="pb-6">
        <h2 className="text-4xl sm:text-5xl font-bold text-white mb-4">Mis Proyectos Destacados</h2>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map((project, index) => (
          <div
            key={index}
            className="bg-gray-800 rounded-lg overflow-hidden border border-gray-700 hover:border-blue-600 transition-all duration-300 transform hover:-translate-y-2 hover:shadow-2xl"
          >
            <img
            ...
            <div className="p-4">
              ...
              <p className="text-gray-400 text-xs sm:text-sm mb-2 line-clamp-3">{project.description}</p>
              <div className="flex flex-wrap gap-1 sm:gap-2 mb-2">
                {project.tech.map((tech, techIndex) => (
                  <span
                    key={techIndex}
                    className="inline-flex items-center rounded-full bg-gray-700 px-2 py-0.5 text-xs font-medium text-gray-200"
                  >
                    {tech}
                  </span>
                ))}
              ...
              <a
                ...
                className="text-blue-400 hover:underline text-sm sm:text-sm font-medium flex items-center gap-1 mt-1"
              >
                Ver Proyecto <span className="ml-1">&rarr;</span>
              </a>
            </div>
          </div>
        ))}
